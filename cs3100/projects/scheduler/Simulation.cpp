...
void Simulation::scheduleJob()
{
  ...
  queue.emplace_back([this]{jobDone(next,timeAllocated)},curTime+timeAllocated+contextSwitchPenalty);
}
...
void Simulation::scheduleIo(int job)
{
  ...
  queue.emplace_back([this](){ioDone(job,finishTime );},curTime+finishTime);
}
