...
<xsl:for-each select="node">
  ...
  <xsl:for-each select="node"><xsl:choose><xsl:when test="@LOCALIZED_STYLE_REF='styles.important'">
\begin{frame}[fragile]
\frametitle{<xsl:value-of select="@TEXT"/>}
\begin{lstlisting}
<xsl:for-each select="node"><xsl:value-of select="@TEXT"/><xsl:text>
...
<xsl:for-each select="hook">
    \includegraphics[width=\textwidth]{<xsl:value-of select="@URI"/>}
    </xsl:for-each>
<xsl:for-each select="attribute"><xsl:if test="@NAME='textbook'">
...
\begin{itemize}
<xsl:for-each select="node/node/node/attribute"><xsl:sort select="@VALUE"/><xsl:if test="position() mod 10 = 0">
\begin{frame}
\frametitle{Textbook sections covered:}
\begin{itemize}
</xsl:if>
...
<xsl:if test="position() mod 10 != 9"><xsl:if test="position() = last()">
\end{itemize}
\end{frame}
</xsl:if></xsl:if>
